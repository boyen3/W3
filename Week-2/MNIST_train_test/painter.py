...
with open("config.json", encoding="utf-8") as f:
    config = json.load(f)
config["model"]["MNIST_ep10"] = load_model(config["model"]["MNIST_ep10"])
# Initialize model
_ = config["model"]["MNIST_ep10"].predict(np.zeros(config["shape"]["MNIST_ep10"]))[0]
